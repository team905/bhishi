...
// This file routes to PostgreSQL if DATABASE_URL is set, otherwise uses SQLite for local dev

// CHECK DATABASE_URL FIRST - before any other requires
const databaseUrl = process.env.POSTGRES_URL || process.env.DATABASE_URL;

if (databaseUrl || process.env.VERCEL) {
  // Use PostgreSQL - immediately export PostgreSQL module
  // Don't load any SQLite code at all
  console.log('[database.js] DATABASE_URL detected, routing to PostgreSQL');
  module.exports = require('./database-postgres');
} else {
  // Use SQLite for local development only
  // Only load sqlite3 when actually needed
  console.log('[database.js] No DATABASE_URL, using SQLite for local development');
  ...
  try {
    ...
    throw new Error('SQLite3 is required for local development. Please install it: npm install sqlite3');
  }
